...
public partial class GridSystem : Manager<GridSystem>
{
    ...
    private async Task InitializeGridObjects(IEnumerable<GridObject> gridObjects)
    {
        foreach (var gridObject in gridObjects)
        {
            ...
            {
                GD.PrintErr($"Could not find grid cell for GridObject {gridObject.Name} at {gridObject.GlobalPosition}");
            }
        }
    }
   ...
   private async Task<List<GridObject>> SetupGrid()
    {
        ...
        {
            GD.PrintErr("GridSystem: InventoryManager is NULL inside SetupGrid. Ensure InventoryManager runs Setup/Execute before GridSystem.");
        }
